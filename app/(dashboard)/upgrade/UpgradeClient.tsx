...
type Plan = 'starter' | 'landlord' | 'portfolio'
...
export function UpgradeClient({ currentPlan, hasSubscription, success, canceled }: UpgradeClientProps) {
  ...
  useEffect(() => {
    if (success) {
      setToast({ msg: 'Subscription activated — welcome to your new plan!', type: 'success' })
    } else if (canceled) {
      setToast({ msg: 'Checkout canceled. No charge was made.', type: 'error' })
    }
  }, [success, canceled])
  ...
  async function handleManageBilling() {
    ...
    try {
      ...
      if (data.url) {
        ...
        setToast({ msg: data.error ?? 'Could not open billing portal', type: 'error' })
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {toast && (
        <div style={{
          ...
          border: toast.type === 'success'
            ...
            : '1px solid rgba(183,110,121,0.40)',
      ...
      {/* Manage billing banner — shown when already subscribed */}
      {hasSubscription && (
        <div style={{
          marginBottom: '24px',
          padding: '16px 24px',
        ...
        {PLANS.map((plan) => {
          ...
          return (
            ...
            >
              ...
              {/* Price */}
              <div style={{ borderTop: '1px solid rgba(201,168,76,0.08)', paddingTop: '20px' }}>
                <p style={{
                  ...
                  {plan.price.split(' ')[0]}
                  <span style={{ fontSize: '14px', color: '#6B6B65', fontFamily: "'Jost', sans-serif", fontWeight: 300, marginLeft: '6px' }}>
                    CAD / mo
                  </span>
                </p>
              </div>
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '10px', flex: 1 }}>
                {plan.features.map((f) => (
                  <li key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px' }}>
                    <Check
                      style={{ width: '13px', height: '13px', color: '#C9A84C', flexShrink: 0, marginTop: '1px' }}
                      strokeWidth={2}
                    />
                    <span style={{
                      fontFamily: "'Jost', sans-serif",
                  ...
                  <button
                    ...
                    onMouseEnter={(e) => {
                      if (!loading) {
                        ...
                        if (!plan.popular) (e.currentTarget as HTMLButtonElement).style.outline = '1px solid rgba(201,168,76,0.55)'
                      }
                    }}
                    onMouseLeave={(e) => {
                      (e.currentTarget as HTMLButtonElement).style.opacity = !!loading && !isLoading ? '0.5' : '1'
                      if (!plan.popular) (e.currentTarget as HTMLButtonElement).style.outline = '1px solid rgba(201,168,76,0.30)'
                    }}
                  >

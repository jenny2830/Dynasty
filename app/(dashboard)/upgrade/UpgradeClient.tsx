    ...
    <div style={{ position: 'relative' }}>
      ...
      {hasSubscription && (
        <div style={{
          ...
          border: `1px solid ${theme.accent}26`, borderRadius: '2px',
            ...
            <div key={plan.key} style={{
              ...
              border: plan.popular
                ...
                : plan.isFree
                  ...
                  : `1px solid ${theme.accent}1F`,
              background: plan.popular ? `${theme.accent}0A` : theme.cardBg,
              boxShadow: plan.popular
                ? `0 4px 32px ${theme.accent}1A, 0 0 0 1px ${theme.accent}14`
                : theme.cardShadow,
              display: 'flex', flexDirection: 'column', gap: '18px',
            }}>
              {/* Corner marks */}
              <div style={{ position: 'absolute', top: '6px', left: '6px', width: '12px', height: '12px', borderTop: `1px solid ${theme.cornerMark}`, borderLeft: `1px solid ${theme.cornerMark}`, pointerEvents: 'none' }} />
              <div style={{ position: 'absolute', bottom: '6px', right: '6px', width: '12px', height: '12px', borderBottom: `1px solid ${theme.cornerMark}`, borderRight: `1px solid ${theme.cornerMark}`, pointerEvents: 'none' }} />

              {plan.popular && (
                <div style={{
                  position: 'absolute', top: '-12px', left: '50%', transform: 'translateX(-50%)',
                  padding: '4px 16px', background: theme.accentGradient,
                  borderRadius: '1px', fontFamily: "'Jost', sans-serif", fontSize: '8px',
                  letterSpacing: '0.22em', textTransform: 'uppercase', color: theme.textOnAccent,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <div style={{
                  ...
                  justifyContent: 'center', border: `1px solid ${theme.accent}2E`,
                  borderRadius: '2px', color: theme.accent, background: `${theme.accent}0F`, flexShrink: 0,
                }}>
                  {plan.icon}
                </div>
                <div>
              ...
              {/* Price */}
              <div style={{ borderTop: `1px solid ${theme.dividerColor}`, paddingTop: '16px' }}>
                <p style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: '30px', fontWeight: 500, letterSpacing: '0.01em', color: theme.textPrimary, margin: 0, lineHeight: 1 }}>
                  {plan.isFree ? (
                    <span>Free</span>
                  ) : (
                    <>
                ...
                {plan.isFree && isCurrent && (
                  <div style={{ marginTop: '10px' }}>
                    ...
                    <div style={{ height: '3px', background: `${theme.accent}26`, borderRadius: '1px', overflow: 'hidden' }}>
                      <div style={{
                        height: '100%', borderRadius: '1px',
                        width: `${(sessionsUsed / FREE_TRIAL_MAX_SESSIONS) * 100}%`,
                        background: remaining > 1 ? theme.accentGradient : theme.valueNegative,
              ...
              <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '8px', flex: 1 }}>
                {plan.features.map((f) => (
                  <li key={f.label} style={{ display: 'flex', alignItems: 'flex-start', gap: '9px' }}>
                    {f.included ? (
                      <Check style={{ width: '12px', height: '12px', color: theme.accent, flexShrink: 0, marginTop: '2px' }} strokeWidth={2.5} />
                    ) : (
                ...
                {isCurrent ? (
                  ...
                  <button
                    ...
                    style={{
                      ...
                      outline: plan.popular ? 'none' : `1px solid ${theme.accent}4D`,
                      boxShadow: plan.popular ? `0 4px 16px ${theme.accent}38` : 'none',
                    }}
                  >
